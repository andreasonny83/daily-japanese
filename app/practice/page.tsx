...
function PracticePageContent() {
  ...
  // Guests get the same self-rate UI as signed-in users, but nothing is
  // persisted (no session to attach progress to) — just a visual pick and
  // advance, same as clicking Skip.
  function handleGuestRate(score: number) {
    if (!card || !revealed || selectedScore !== null) return;
    setSelectedScore(score);
    setTimeout(() => loadCard("level", selectedLevelId), RATE_ADVANCE_DELAY_MS);
  }
  ...
  useEffect(() => {
    if (!revealed || !card || selectedScore !== null) return;
    function handleKeyDown(e: KeyboardEvent) {
      ...
      (isAuthed ? handleRate : handleGuestRate)(Number(e.key));
    }
        ...
        <div className="px-6 pb-6 md:px-8 md:pb-8">
          {revealed && card && (
            <FeedbackButtons
              ...
              onSelect={isAuthed ? handleRate : handleGuestRate}
            />
          )}

          {!isAuthed && (
            <div className="mt-3 rounded-lg border border-dashed border-gray-200 bg-gray-50 px-4 py-3 text-center text-xs text-gray-500">
              <Link
                href="/auth/sign-in"
              ...
              to save your progress and unlock leveled learning.
            </div>
          )}
